    ...
    if data is not None:
        ...
        if isinprivatechannel.content.lower() == "yes":
            ...
            if isHostedByMat.content.lower() == "yes":
                ...
                with open("/home/gdps/GDPS_Creator/user-data.json", "r+") as data_file:
                    user_data = json.load(data_file)
                userpass = "null"
                for key in user_data:
                    if user_data[key]["gdps_custom_url"] == customurl.content:
                        userpass = user_data[key]["password"]
                if userpass == "null":
                    await ctx.send("This custom URL is not valid")
                    return
                patchedurl = f"gdps_{customurl.content}"
                try:
...
async def updateinfo(ctx):
    ...
    with open("StatsDatabase.json", "r+") as file:
        ...
        if data[str(serverid)]["mysqlhost"] == "127.0.0.1":
            with open("/home/gdps/GDPS_Creator/user-data.json", "r+") as creator_file:
                creator_data = json.load(creator_file)
                if str(userid) in creator_data:
                    data[str(serverid)]["password"] = creator_data[str(userid)]["password"]
                    json.dump(data, file, indent=4)
